...
(function () {
  ...
  async function runFlow() {
    ...
    try {
      ...
      if (!step1Res.ok) {
        throw new Error(
          `第 1 步请求失败: ${step1Res.status} ${step1Res.statusText}\n${formatResult(step1Data)}`,
        );
      }
      ...
      if (!step2Res.ok) {
        throw new Error(
          `第 2 步请求失败: ${step2Res.status} ${step2Res.statusText}\n${formatResult(step2Data)}`,
        );
      }
  ...
  function createPanel() {
    ...
    document
      .getElementById(COPY_BUTTON_ID)
      ?.addEventListener("click", copyResult);
    document
      .getElementById(COPY_API_KEY_BUTTON_ID)
      ?.addEventListener("click", copyApiKeyValue);
    document
      .getElementById(CLEAR_BUTTON_ID)
      ?.addEventListener("click", clearResult);
    document
      .getElementById(TOGGLE_BUTTON_ID)
      ?.addEventListener("click", togglePanelBody);
  }
