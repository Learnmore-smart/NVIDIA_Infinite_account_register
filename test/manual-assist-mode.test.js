...
test('runner activates only cookie consent, verified email Next, the sole account action, passkey Later, passkey skip confirm, developer-consent submit, and cloud-account create', () => {
  // +1 for optional auto email-code submit click when Gmail fetcher is configured.
  assert.ok([...runnerSource.matchAll(/\.click\s*\(/g)].length >= 7);
  ...
  assert.match(runnerSource, /handleCaptchaIntervention[\s\S]*?resubmitAccountActionAfterCaptcha|submitVisibleAccountAction[\s\S]*?handleCaptchaIntervention[\s\S]*?submitVisibleAccountAction/);
